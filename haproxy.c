...
int event_accept(int fd) {
    ...
    printf("event_accept_fd=>%d,owner%d\n",fd,p);
    struct task *s;
    int laddr;
    int cfd;
    int one = 1;
        ...
        while (ptr < req->r) {
            ...
            if (ptr < req->r) {
                ...
                if (ptr == req->h) { /* empty line, end of headers */
                    ...
                    if (mode & MODE_DEBUG) {
                        int len, max;
                        len = sprintf(trash, "clihdr[%04x:%04x]: ", (unsigned short) t->cli_fd,
                                      (unsigned short) t->srv_fd);
                        max = ptr - req->h;
                        if (max > sizeof(trash) - len - 2)
                            max = sizeof(trash) - len - 2;
                        strncat(trash + len, req->h, max);
                        len += max;
                        trash[len++] = '\n';
                        trash[len] = '\0';
        ...
        while (ptr < rep->r) {
            ...
            if (ptr < rep->r) {
                ...
                if (ptr == rep->h) { /* empty line, end of headers */
                    ...
                    if (mode & MODE_DEBUG) {
                        int len, max;
                        len = sprintf(trash, "srvhdr[%04x:%04x]: ", (unsigned short) t->cli_fd,
                                      (unsigned short) t->srv_fd);
                        max = ptr - rep->h;
                        if (max > sizeof(trash) - len - 2)
                            max = sizeof(trash) - len - 2;
                        strncat(trash + len, rep->h, max);
                        len += max;
                        trash[len++] = '\n';
                        trash[len] = '\0';
                        write(1, trash, len);
    ...
    while (argc > 0) {
        ...
        if (**argv == '-') {
            ...
            if (*flag == 'v') {
                ...
                switch (*flag) {
                    case 'n' :
                        cfg_maxconn = atol(*argv);
                        break;
                    case 'N' :
                        cfg_maxpconn = atol(*argv);
                        break;
                    case 'f' :
                        cfg_cfgfile = *argv;
                        break;
